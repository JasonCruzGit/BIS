...
import { ArrowLeft, Save, Upload, X, User, FileText } from 'lucide-react'
...
export default function EditOfficialPage() {
  ...
  const { data: official, isLoading: officialLoading } = useQuery(
    ['official', officialId],
    async () => {
      const { data } = await api.get(`/officials/${officialId}`)
      return data
    },
    {
      enabled: !!officialId,
      onSuccess: (data) => {
        if (data) {
          setFormData({
            ...
            termStart: data.termStart ? new Date(data.termStart).toISOString().split('T')[0] : '',
            termEnd: data.termEnd ? new Date(data.termEnd).toISOString().split('T')[0] : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/officials/${officialId}`, submitData, {
        headers: {
      ...
      queryClient.invalidateQueries(['official', officialId])
      router.push('/officials')
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Failed to update official')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (officialLoading) {
    return (
      <Layout>
        <div className="flex items-center justify-center h-64">
          <div className="text-gray-500">Loading official data...</div>
        </div>
      </Layout>
    )
  }

  const displayPhoto = photoPreview || (existingPhoto ? getFileUrl(existingPhoto) : null)
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Edit Official</h1>
              <p className="mt-1 text-gray-600">Update official information</p>
            </div>
          </div>
        </div>
            ...
            {existingDocuments.length > 0 && (
              <div className="mb-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {existingDocuments.map((doc: string, index: number) => (
                    <div key={index} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <a
                        ...
                        className="text-xs text-primary-600 hover:text-primary-700 truncate block text-center"
                      >
                        View {index + 1}
                      </a>
                    </div>
